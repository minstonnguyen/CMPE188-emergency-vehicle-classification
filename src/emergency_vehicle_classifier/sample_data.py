...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a small synthetic dataset for smoke tests.")
...
def _draw_vehicle(draw: ImageDraw.ImageDraw, size: int, label: str, rng: random.Random) -> None:
    ...
    if stripe is not None:
        ...
        bar_top = roof[1] - 4
        bar_bottom = roof[1] + 3
        draw.rectangle((bar_left, bar_top, (bar_left + bar_right) // 2, bar_bottom), fill=light_left)
        draw.rectangle((((bar_left + bar_right) // 2), bar_top, bar_right, bar_bottom), fill=light_right)

    wheel_r = 7
    for wheel_x in (left + int(car_w * 0.22), right - int(car_w * 0.22)):
        draw.ellipse((wheel_x - wheel_r, bottom - 2, wheel_x + wheel_r, bottom + 12), fill=(20, 20, 20))
